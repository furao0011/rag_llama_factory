...
from config.RAGconfig import RAG
if TYPE_CHECKING:
...
class WebChatModel(ChatModel):
    ...
    def stream(
        ...
        if use_rag:
            ...
            messages[-1]["content"] = process_input(original_content)
            print('THIS is true input:' + messages[-1]["content"])
            if content_rag:
                chatbot.append([f'*RAG prompt:*\n```\n{messages[-1]["content"]}\n```', None])  # 在聊天框中展示加工后的内容
        chatbot[-1][1] = ""
        response = ""
