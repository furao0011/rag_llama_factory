...
def create_chat_box(
    ...
    with gr.Column(visible=visible) as chat_box:
        ...
        with gr.Column(scale=6):
            with gr.Row():
                with gr.Column():
                    use_rag = gr.Checkbox(label="Use RAG Strategy", value=False, interactive=True)
                with gr.Column():
                    content_rag = gr.Checkbox(label="Show RAG Content", value=False, interactive=True)
    ...
    submit_btn.click(
        ...
        [chatbot, messages, system, tools, image, video, max_new_tokens, top_p, temperature, use_rag, content_rag],
        [chatbot, messages],
    )
